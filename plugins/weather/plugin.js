...
executeMethod = function(request, response, arguments) {
    var YQL = require('yqlp');

    if (arguments == undefined || arguments.location == undefined) {
        location = "DEN";
    } else {
...
execute = async(function(request, response, arguments) {
    ...
    var result = "";
    var data = await(executeMethod(request, response, arguments));
    try {
        ...
        result = "<div>Weather is unavailable</div>";
    }
    return result; 
});
